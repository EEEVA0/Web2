...
## 11. Development Workflow and Version Control

Git is used to track development progress.
...
git status
git add .
git commit -m "Implement crawler with internal link discovery"
git commit -m "Add inverted index with TF-IDF statistics"
git commit -m "Add command-line interface commands"
git commit -m "Add tests for crawler, indexer, and search"
git commit -m "Update README with setup and design rationale"
```

The commit history should demonstrate regular, incremental development rather than a single final commit. This supports the version control requirements of the coursework.
...
The project separates responsibilities across four main modules:

```
crawler.py  →  crawling and text extraction
indexer.py  →  tokenisation, indexing, storage
search.py   →  query processing and ranking
main.py     →  command-line interaction
```

This separation makes the implementation easier to explain, test, debug, and extend.

---

## 13. Known Limitations

This is a coursework-scale search engine tool, so it has some limitations:

- It only crawls the required target website.
- It does not implement a full web-scale crawler.
- It does not support Boolean OR queries.
- It does not support phrase search with exact word order.
- It does not use stemming or lemmatisation.
- It does not remove stop words.
- It does not include spelling correction or query suggestions.

These features could be added in future extensions.
...
- HTML report generation for search results;
- more advanced ranking models.

---

## 15. GenAI Declaration Note

This assessment allows the use of Generative AI tools, but any use must be declared and critically evaluated in the video demonstration.

If GenAI was used, the video should explain:

- which tool was used;
- what it was used for;
- where it helped;
- where it gave incorrect or incomplete suggestions;
- how the generated code or explanations were checked;
- how using GenAI affected the development process and learning.

Example reflection:

> I used GenAI to help reason about the structure of an inverted index and to generate initial examples of unit tests. However, I found that some AI-generated suggestions only stored a list of URLs and did not include required word statistics such as frequency and positions. I modified the design to store page-level postings with frequency, positions, term density, and TF-IDF. This helped me understand why an inverted index needs to store more than just page links.

All submitted code should be understood and explainable by the author.

---

## 16. Video Demonstration Checklist

The 5-minute video demonstration should include:
...
Briefly explain the testing strategy.
...
Briefly explain the incremental development process.

### GenAI Critical Evaluation

Explain how GenAI was or was not used, with specific examples.

---

## 17. Submission Notes

The final Minerva submission should include a single PDF or TXT document containing:

- Video demonstration link.
- GitHub repository URL.
- Index file submission or download link.
...
- `requirements.txt`;
- `README.md`;
- generated index file or instructions for generating it.

Before submission, check that:

- the video link is accessible;
- the GitHub repository is public or accessible to markers;
- the index file is included or linked;
